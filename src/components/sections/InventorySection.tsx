    ...
    <SectionWrapper id="inventory" className="bg-navy-900">
      ...
      <div className="relative mx-auto max-w-7xl px-6">
        ...
        <p className="mb-12 max-w-2xl font-body text-gray-400">
          Every adventurer needs the right gear. Here&apos;s what I carry into battle —
          languages, frameworks, tools, and the soft skills that make the difference.
        </p>
        ...
        <div className="mb-8 flex flex-wrap gap-3">
          {inventoryCategories.map((cat) => (
            ...
            >
              <span>{cat.icon}</span>
              {cat.title}
              <span className="text-[7px] opacity-60">({cat.subtitle})</span>
            </button>
          ))}
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          {/* Items grid */}
          <div ref={gridRef} className="grid grid-cols-2 gap-4 lg:col-span-2">
            {category.items.map((item) => (
              <button
                key={item.id}
                onClick={() => handleItemClick(item)}
                className={`inventory-slot group rounded-lg p-4 text-left ${
                  selectedItem?.id === item.id ? "border-cyan-400/60 glow-cyan" : ""
                }`}
              >
                <div className="mb-3 flex items-center justify-between">
                  ...
                  <span className="font-pixel text-[7px] text-gold-400">Lv.{item.level}</span>
                </div>
          ...
          {/* Detail panel */}
          <div className="pixel-border item-detail rounded-lg bg-navy-800/50 p-6">
            {selectedItem ? (
              <>
                ...
                <div className="mb-2 flex justify-between font-pixel text-[8px]">
                  ...
                  <span className="text-cyan-400">{selectedItem.level}/100</span>
                </div>
